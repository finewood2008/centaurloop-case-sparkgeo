...
The memory workspace is intentionally visible beside the conversation so the user can watch profile facts, imported knowledge, and approved lessons change while the loop runs.
...
记忆工作区会常驻在对话右侧，让用户实时看到企业档案、导入资料和复盘经验的变化。
