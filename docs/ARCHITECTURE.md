...
图片生成引擎和模型名称放在设置里的“模型与集成”，风格、比例和提示词偏好放在“图片生成”。当前实现使用本地确定性 SVG 预览。生产适配器可以把 `articleImageService` 替换为真实图片生成 API，同时保持 UI 契约不变。
