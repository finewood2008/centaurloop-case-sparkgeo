...
- **BYOK runtime**: an OpenAI-compatible text model configured in Settings -> BYOK with the user's own key, base URL, and model name.
...
- **BYOK 运行时**：在设置里的“BYOK”标签页填写用户自己的 Key、Base URL 和模型名称，接入 OpenAI-compatible 文本模型。
...
图片生成也走 BYOK 配置：图片供应商/引擎、图片 API Key、Base URL 和模型名称放在设置里的“BYOK”标签页；风格、比例和提示词偏好放在“图片生成”。当前实现使用本地确定性 SVG 预览。生产适配器可以把 `articleImageService` 替换为真实图片生成 API，同时保持 UI 契约不变。
