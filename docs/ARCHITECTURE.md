...
- **BYOK runtime**: an OpenAI-compatible text model configured in Settings -> BYOK with the user's own text key, text base URL, and text model name.
...
- Engine, model, style, and ratio metadata

Image generation also uses BYOK configuration, but with separate credentials from text generation: image provider/engine, image API key, image Base URL, and image model name live under Settings -> BYOK. Style, ratio, and prompt preferences live under Settings -> Image Generation. The current implementation uses a local deterministic SVG preview. A production adapter can replace `articleImageService` with a real image generation API while keeping the same UI contract.
...
- **BYOK 运行时**：在设置里的“BYOK”标签页填写用户自己的文本 Key、文本 Base URL 和文本模型名称，接入 OpenAI-compatible 文本模型。
...
图片生成也走 BYOK 配置，但凭据与文本生成完全独立：图片供应商/引擎、图片 API Key、图片 Base URL 和图片模型名称放在设置里的“BYOK”标签页；风格、比例和提示词偏好放在“图片生成”。当前实现使用本地确定性 SVG 预览。生产适配器可以把 `articleImageService` 替换为真实图片生成 API，同时保持 UI 契约不变。
